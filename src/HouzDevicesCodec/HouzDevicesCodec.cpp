...
unsigned long HouzDevicesCodec::encode(int _cmd, int deviceId, unsigned long devicePayload)
{
...
deviceData HouzDevicesCodec::decode(unsigned long rawData, unsigned long nodeId) {
	deviceData decoded;
	decoded.raw = rawData;
...
//////////////////////////////////////////////////////
// Pressure patch
unsigned long HouzDevicesCodec::pressureEncode(float devicePayload){
	unsigned long ret = (devicePayload-900)*100; //900hPa offset
  if(ret>0xFFFF) ret=0xFFFF;
	return ret;
}

float HouzDevicesCodec::pressureDecode(unsigned long devicePayload){
